...
pub fn salsa20_expansion_16(k: [u8; 16], n: [u8; 16]) -> [u8; 64] {
    let mut r = [0; 64];
...
pub fn salsa20_expansion_32(k0: [u8; 16], k1: [u8; 16], n: [u8; 16]) -> [u8; 64] {
    let mut r = [0; 64];
...
pub fn salsa20_decrypt_16(k: [u8; 16], v: [u8; 8], enc: Vec<u8>) -> Vec<u8> {
    let mut r = vec![0; enc.len()];
    ...
    loop {
        if i >= enc.len() / 64 {
            ...
            let counter: [u8; 8] = little_endian_inv64(i as u64);
            for j in 0..8 {
                vi[j+8] = counter[j];
            }

            // generate salsa20 expansion
            let bytes = salsa20_expansion_16(k, vi);

            // copy generated bytes to output whilst XORing them with the
            // provided message truncating the length to m.len().
            if enc.len() >= (i * 64 + 64) {
                for j in 0..64 {
                    r[i * 64 + j] = bytes[j] ^ enc[i * 64 + j];
                }
            } else {
...
pub fn salsa20_encrypt_16(k: [u8; 16], v: [u8; 8], m: Vec<u8>) -> Vec<u8> {
...
pub fn salsa20_encrypt_32(k0: [u8; 16], k1: [u8; 16], v: [u8; 8], m: Vec<u8>) -> Vec<u8> {
    let mut r = vec![0; m.len()];
    let mut i = 0;
    loop {
        if i >= (m.len() / 64) {
            ...
            // generate salsa20 expansion
            let bytes = salsa20_expansion_32(k0, k1, vi);
...
mod tests {
    ...
    fn test_salsa20_expansion_16() {
        ...
        let r1 = salsa20_expansion_16(k0, n);
        let mut r1_result = true;
        for i in 0..64 {
            if r1[i] != test_v1[i] {
    ...
    fn test_salsa20_expansion_32() {
        ...
        let r1 = salsa20_expansion_32(k0, k1, n);
        assert_eq!(bytes_eq(r1.to_vec(), test_v1), true);
    }

    #[test]
    fn test_salsa20_encrypt_16() {
        ...
        let m = vec![0; 512];
        // encrypt message
        let r1 = salsa20_encrypt_16(k, v, m.clone());
        // decrypt message
        let m1 = salsa20_encrypt_16(k, v, r1.clone());
    ...
    fn test_salsa20_encrypt_32() {
        ...
        let m = vec![0; 512];
        // encrypt message
        let r1 = salsa20_encrypt_32(k0, k1, v, m.clone());
        // decrypt message
        let m1 = salsa20_encrypt_32(k0, k1, v, r1.clone());
        let d1 = xor_digest(r1.clone());
        let test_s1_xor_digest = vec![
